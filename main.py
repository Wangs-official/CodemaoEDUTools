...
def CreateParser():
    ...
    followuser_parser.add_argument(
        "-uid", "--user-id", required=True, help="训练师编号"
    )
    ...
    sendrevietowork_parser.add_argument(
        "-wid", "--work-id", required=True, help="作品ID"
    )

    sendrevietowork_parser.add_argument(
        "-r", "--review-text", required=True, help="评论内容"
    )
...
if __name__ == "__main__":
    ...
    if args.command == "report-work":
        logging.info("请稍后...")
        if ReportWork(
            args.token_file, args.work_id, args.report_reason, args.report_describe
        ):
            logging.info("执行成功")
