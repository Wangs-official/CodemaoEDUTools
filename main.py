...
def ReportWork(Path: str, WorkID: str, Reason: str, Describe: str) -> bool:
    if not os.path.exists(Path):
        ...
        def CallToAPI_ReportWork(Token: str) -> bool:
            try:
                response = PostAPI(Path="/nemo/v2/report/work",
...
def CreateParser():
    ...
    signature_parser = subparsers.add_parser(  # noqa: F841
        "signature", help="签订友好协议，推荐在使用其他功能前统一签订一次友好协议，防止出现无法请求的情况", )
    ...
    likework_parser.add_argument("-wid", "--work-id", nargs='+', required=True, help="作品ID")

    # CollectionWork(Path: str, WorkID: str)
    collectwork_parser = subparsers.add_parser("collect-work", help="批量收藏一个作品")

    collectwork_parser.add_argument("-wid", "--work-id", nargs='+', required=True, help="作品ID")
    ...
    reportwork_parser.add_argument("-wid", "--work-id", nargs='+', required=True, help="作品ID")
    ...
    sendrevietowork_parser.add_argument("-r", "--review-text", required=True, nargs='+', help="评论内容")
    ...
    createstudentedu_parser.add_argument("-sl", "--student-name-list", required=False, type=json.loads,
                                         default=student_names,
                                         help="学生名字的列表，最多100个学生，学生命名遵循官方命名规则", )

    createstudentedu_parser.add_argument("-o", "--output-xls", required=False, default="output.xls",
    ...
    mergestudentxls_parser.add_argument("-o", "--output-xlsx", required=False, default="output_xls",
